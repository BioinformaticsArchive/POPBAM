...
static inline void insert_offset(khash_t(i) *h, int bin, uint64_t beg, uint64_t end)
{
	...
	if (ret)
	{
		l->m = 1;
		l->n = 0;
		l->list = (pair64_t*)calloc((size_t)l->m, 16);
	}
	if (l->n == l->m)
	{
		l->m <<= 1;
		l->list = (pair64_t*)realloc(l->list, (size_t)l->m * 16);
	}
	l->list[l->n].u = beg;
	l->list[l->n++].v = end;
}
...
static inline void insert_offset2(bam_lidx_t *index2, bam1_t *b, uint64_t offset)
{
	...
	if (index2->m < end+1)
	{
		...
		index2->m = end + 1;
		kroundup32(index2->m);
		index2->offset = (uint64_t*)realloc(index2->offset, (size_t)index2->m * 8);
...
static void merge_chunks(bam_index_t *idx)
{
	...
	for (i=0; i < idx->n; ++i)
	{
		...
		{
			if (!kh_exist(index, k) || (kh_key(index, k) == BAM_MAX_BIN))
				continue;
			p = &kh_value(index, k);
			m = 0;
			for (l=1; l < p->n; ++l)
...
bam_index_t *bam_index_core(bamFile fp)
{
	...
	while ((ret = bam_read1(fp, b)) >= 0)
	{
		...
		if (c->bin != last_bin)
		{
			...
			if (last_bin == 0xffffffffu && save_tid != 0xffffffffu)
			{
				off_end = last_off;
	...
	if (ret < -1)
		fprintf(stderr, "[bam_index_core] truncated file? Continue anyway. (%d)\n", ret);
	free(b->data);
	free(b);
...
int bam_index(int argc, char *argv[])
{
	...
	if (argc >= 3)
		bam_index_build2(argv[1], argv[2]);
	else
...
int bam_idxstats(int argc, char *argv[])
{
	...
	if (fp == 0)
	{
		fprintf(stderr, "[%s] fail to open BAM.\n", __FUNCTION__);
		return 1;
	}
	...
	if (idx == 0)
	{
		fprintf(stderr, "[%s] fail to load the index.\n", __FUNCTION__);
		return 1;
	}
	for (i=0; i < idx->n; ++i)
	{
